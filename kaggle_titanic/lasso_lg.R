...
set.seed(1)
lasso <- cv.glmnet(x = X_train |> as.matrix(), y = y_train |> as.matrix(),
                   type.measure="mse", alpha=1, family="binomial")
plot(lasso)
?cv.glmnet

### PREDICTION
lasso.predicted <- predict(lasso, lasso$lambda.1se,
                           newx = X_test |> as.matrix(),
                           type = 'response')
survived <- rep(0, nrow(X_test))
survived[lasso.predicted > 0.5] <- 1
...
test |>
  ...
  write_csv("data/lasso_predicted.csv")
